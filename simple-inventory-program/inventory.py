...
def show():
    ...
    if (var1.get()):
        ...
        qty = int(e6.get())
        tot = int(price * qty)
        tempList = [['Big Minute Maid', e1.get(), e6.get(), tot]]
    ...
    if (var2.get()):
        ...
        qty = int(e7.get())
        tot = int(price * qty)
        tempList = [['Big Water', e2.get(), e7.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var3.get()):
        ...
        qty = int(e8.get())
        tot = int(price * qty)
        tempList = [['Beer', e3.get(), e8.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var4.get()):
        ...
        qty = int(e9.get())
        tot = int(price * qty)
        tempList = [['Soda', e4.get(), e9.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
    ...
    if (var5.get()):
        ...
        qty = int(e10.get())
        tot = int(price * qty)
        tempList = [['Kampako', e5.get(), e10.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
...
Label(root, text="Amara's Stock", font="arial 22 bold" ,bg="white").place(x=5, y=10)


var1 = IntVar()
Checkbutton(root, text="Big Minute Maid", variable=var1).place(x=10, y=50)

var2 = IntVar()
Checkbutton(root, text="Big Water", variable=var2).place(x=10, y=80)

var3 = IntVar()
Checkbutton(root, text="Beer", variable=var3).place(x=10, y=110)

var4 = IntVar()
Checkbutton(root, text="Soda", variable=var4).place(x=10, y=140)

var5 = IntVar()
Checkbutton(root, text="Kampako", variable=var5).place(x=10, y=170)
